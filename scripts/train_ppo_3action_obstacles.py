...
# 3-action 대조군 학습 스크립트 (2-action 버전과 별개 파일).
# 핵심 차이: make_env에서 ActionWrapper를 적용하지 않아 정책이 네이티브 3D
# [steering, gas, brake]를 출력한다. PPOAgent는 action_dim을 action_space에서
# 자동 결정하므로 ppo_agent_2(per-minibatch KL early-stop, 네트워크 동일)를 재사용한다.
from src.ppo_agent_2 import PPOAgent
...
config = {
    ...
    "log_interval": 1,                  
    "save_interval": 10,                
    "save_dir": "./models/ppo_3action_obstacles",    # 3-action 대조군 (2-action과 분리)
    "log_dir": "./logs/ppo_3action_obstacles",

    # 3-action 베이스(장애물 없는 트랙에서 clean~675)에서 파인튜닝.
    # (--checkpoint 로 변경 가능. 반드시 3-action 체크포인트여야 함 — size mismatch 방지)
    "checkpoint_path": "./BestSavedAgents/evaluated641.pth",
...
def make_env(env_id: str, seed: int, frame_stack: int, max_episode_steps: int, idx: int = 0):
    def _init():
        ...
        # 3-action: ActionWrapper 미적용 → 정책이 네이티브 3D [steer, gas, brake] 출력.
        # (RewardShapingWrapper는 이 위(안쪽)라 어차피 3D action을 받으므로 accel-turn 동일.)
